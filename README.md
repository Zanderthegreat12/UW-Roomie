...
How to host code on independent server (for developers):
  ...
  4. host code in backend folder on a server using the following commands:
    - to build the code, run "mvn package -Dmaven.test.skip"
    - to run the code, run "mvn compile exec:java -Dexec.mainClass=roomieapp.Server"
  5. Anytime the https://5pfrmumuxf.us-west-2.awsapprunner.com is written in frontend code, change it to your new server's domain
